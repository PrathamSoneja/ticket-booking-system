...
def cluster():
    ...
    pb_grpc.add_ClientServiceServicer_to_server(ClientServer(app=app), leader)
    leader_port = leader.add_insecure_port("127.0.0.1:0")
    leader.start()
    follower = grpc.server(futures.ThreadPoolExecutor(max_workers=4))
    pb_grpc.add_ClientServiceServicer_to_server(ClientServer(app=app, is_leader=False, leader_address=f"127.0.0.1:{leader_port}"), follower)
...
def test_client_leader_redirection(cluster) -> None:
    ...
    assert client.server_address == leader
    client.close()
